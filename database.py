...
class Database:
    ...
    def add_authors(self, name, birth_year):
        with self.conn:
            self.conn.execute(
    ...
    def get_author(self, name):
        ...
        cursor.execute('SELECT id, name, birth_year FROM authors WHERE name = ?'),
        (name)
    ...
    def find_book_by_author(self, author_name):
        cursor = self.conn.cursor()
        cursor.execute('''
            SELECT books.title, author_name, books.publication_year 
            FROM books
    ...
    def list_books(self):
        cursor = self.conn.cursor()
        cursor.execute('''
                SELECT books.title, authors.name, books.publication.birth_year
                FROM books
                JOIN authors 
